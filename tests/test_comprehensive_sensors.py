...
class TestComprehensiveRawValuePassthrough:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                "mph",
                13,
                "Should normalize mph to km/h since Torque sends metric",
            ),
    ...
    def test_sensor_raw_unit_passthrough(self, name, unit, pid, description):
        """Test that all sensor types preserve their raw units without conversion, except mph which is normalized to km/h."""
        sensor = TorqueSensor(name, unit, pid, "TestCar", {})

        # mph should be normalized to km/h since Torque always sends metric values
        expected_unit = "km/h" if unit == "mph" else unit

        # The unit should be passed through exactly as received from Torque (except mph -> km/h)
        assert (
    ...
    def test_all_sensor_types_accept_all_numeric_values(self):
        ...
        for name, unit, pid in sensor_types:
            ...
            for test_val in test_values:
                assert (
                    sensor._is_value_valid(test_val) is True
    ...
    def test_database_efficiency_preserved_for_all_types(self):
        ...
        for (
            ...
            pid,
            expected_threshold,
        ) in sensor_types_with_expected_thresholds:
            sensor = TorqueSensor(name, unit, pid, "TestCar", {})
