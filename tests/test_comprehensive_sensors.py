...
class TestComprehensiveRawValuePassthrough:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("Coolant Temperature", "°F", 5, UnitOfTemperature.FAHRENHEIT, SensorDeviceClass.TEMPERATURE, "Should normalize and set device class"),
            ("Air Intake Temperature", "°C", 15, UnitOfTemperature.CELSIUS, SensorDeviceClass.TEMPERATURE, "Should normalize and set device class"),
            ("Oil Temperature", "K", 23, "K", None, "Should preserve unknown unit"),
            # Speed sensors - should normalize and set device class
            ("Vehicle Speed", "mph", 13, UnitOfSpeed.MILES_PER_HOUR, SensorDeviceClass.SPEED, "Should normalize and set device class"),
            ("Vehicle Speed", "km/h", 13, UnitOfSpeed.KILOMETERS_PER_HOUR, SensorDeviceClass.SPEED, "Should normalize and set device class"),
            ("Vehicle Speed", "kmh", 13, UnitOfSpeed.KILOMETERS_PER_HOUR, SensorDeviceClass.SPEED, "Should normalize variant"),
            # Pressure sensors - should normalize and set device class
            ("Intake Manifold Pressure", "kPa", 11, UnitOfPressure.KPA, SensorDeviceClass.PRESSURE, "Should normalize and set device class"),
            ("Fuel Pressure", "psi", 10, UnitOfPressure.PSI, SensorDeviceClass.PRESSURE, "Should normalize and set device class"),
            ("Barometric Pressure", "bar", 51, UnitOfPressure.BAR, SensorDeviceClass.PRESSURE, "Should normalize and set device class"),
            # Voltage sensors - should normalize and set device class
            ("Battery Voltage", "V", 14, UnitOfElectricPotential.VOLT, SensorDeviceClass.VOLTAGE, "Should normalize and set device class"),
            ("Battery Voltage", "volt", 14, UnitOfElectricPotential.VOLT, SensorDeviceClass.VOLTAGE, "Should normalize variant"),
            # Distance sensors - should normalize and set device class
            ("Trip Distance", "miles", 161, UnitOfLength.MILES, SensorDeviceClass.DISTANCE, "Should normalize and set device class"),
            ("Trip Distance", "km", 161, UnitOfLength.KILOMETERS, SensorDeviceClass.DISTANCE, "Should normalize and set device class"),
            # No normalization for these
            ("Engine RPM", "rpm", 12, "rpm", None, "Should preserve RPM"),
            ("Engine Load", "%", 4, "%", None, "Should preserve percentage"),
            ("Custom Sensor", "custom_unit", 200, "custom_unit", None, "Should preserve custom units"),
        ],
    )
